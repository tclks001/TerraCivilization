...
void APlanetTopologyDebugMesh::Rebuild()
{
    ...
    if (Material)
    {
        ...
        //
        // R4 期望材质里至少有一个 Custom 节点同时拥有以下 Inputs：
        //   UV0, UV1, UV2, UV3, WorldPos, PlanetCenter, CellAttrLUT, CellDirLUT
        // 缺失任意一项 → 打印 Error 级别日志（说明用户挂了 R3 / R2 旧材质，或新材质没接全）
        ...
        if (UMaterial* BaseMat = Material->GetMaterial())
        {
            ...
            for (UMaterialExpression* Expr : Exprs)
            {
                if (UMaterialExpressionCustom* Custom = Cast<UMaterialExpressionCustom>(Expr))
                {
                    ...
                    for (const FString& Need : ExpectedR4Inputs)
                    {
                    ...
                    {
                        bAnyR4Compliant = true;
                        UE_LOG(LogPlanetTopologyDebugMesh, Warning,
                            TEXT("    ✓ R4 compliance: ALL %d expected inputs present & connected"),
                            ExpectedR4Inputs.Num());
                    }
                    else
                    {
                        if (Missing.Num() > 0)
                        {
                            UE_LOG(LogPlanetTopologyDebugMesh, Error,
                                TEXT("    ✗ R4 missing inputs: [%s] — this is likely an R2/R3 material, not R4"),
                                *FString::Join(Missing, TEXT(", ")));
                        }
                        if (Disconnected.Num() > 0)
                        {
                            UE_LOG(LogPlanetTopologyDebugMesh, Error,
                                TEXT("    ✗ R4 inputs declared but NOT connected: [%s]"),
            ...
            else if (!bAnyR4Compliant)
            {
                UE_LOG(LogPlanetTopologyDebugMesh, Error,
                    TEXT("[PlanetTopologyDebugMesh] Material '%s' has %d Custom nodes but NONE is R4-compliant. ")
                    TEXT("Expected one Custom with inputs: UV0,UV1,UV2,UV3,WorldPos,PlanetCenter,CellAttrLUT,CellDirLUT. ")
                    TEXT("See R4_VoronoiBoundary.md §5 for the exact wiring."),
                    *BaseMat->GetName(), CustomFound);
            }
            else
            {
                UE_LOG(LogPlanetTopologyDebugMesh, Log,
                    TEXT("[PlanetTopologyDebugMesh] Material '%s' is R4-compliant ✓"),
                    *BaseMat->GetName());
            }
        }
#endif
    ...
    UE_LOG(LogPlanetTopologyDebugMesh, Log,
        TEXT("[PlanetTopologyDebugMesh] Rebuilt (R4: argmax(dot(dir,V_i)) + dual LUT). ")
        TEXT("SubdivisionLevel=%d  Cells=%d  Corners=%d  Verts=%d  Tris=%d  Radius=%.1f  Smooth=%s  ")
        TEXT("CellAttrLUT=%s  CellDirLUT=%s  PlanetCenter=(%.1f,%.1f,%.1f)  NumLayersHint=%d"),
