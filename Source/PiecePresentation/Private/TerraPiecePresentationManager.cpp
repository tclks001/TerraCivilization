    ...
    const FTerraPieceVisualConfig& VisualConfig)
{
    ...
    for (const FTerraPiecePresentationStep& Step : Steps)
    {
        ...
        if (!IsValid(PieceActor))
        {
            ...
            PieceActor = World->SpawnActor<ATerraPieceActor>(ATerraPieceActor::StaticClass(), Snapshot.WorldTransform, SpawnParams);
            if (!PieceActor)
            {
    ...
    UE_LOG(LogTerraPiecePresentation, Verbose,
        TEXT("[PiecePresentation][P1] Sync pieces. Spawned=%d Updated=%d Removed=%d AliveActors=%d"),
        SpawnedCount,
        UpdatedCount,
        RemovedCount,
        PieceActors.Num());
