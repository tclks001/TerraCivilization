...
>
> G6 目标是把“军旗被吃 -> 阵营失败 -> 跳过失败阵营 -> 只剩一方时宣布胜利”这条闭环补齐，并直接映射到当前 `FTerraGameplayContainer` 的 C++ 实现。
...
例如一次行动同时命中两个敌方军旗，则两个阵营都立即失败并清盘。

### 5.3 失败阵营的军旗自身

军旗和本回合其他被吃棋子一样，先在 `ResolvePendingCaptures_()` 中被移除；
