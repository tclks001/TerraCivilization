...
P4.5 Rider 马背 Socket / 骨骼跟随阶段设计见：[P4_5MountedRiderSaddleAttachDesign.md](P4_5MountedRiderSaddleAttachDesign.md)。
...
详细阶段设计见：[P5WeaponAttachmentDesign.md](P5WeaponAttachmentDesign.md)。
...
- 骑兵挂斧类武器。
- 主将保持空手施法。
- 武器跟随角色骨骼 Socket，不参与 Gameplay 判定。
- 武器模型、Attach Name、相对位置 / 旋转 / 缩放均暴露为 UPROPERTY，允许在编辑器里按当前资产校准。

验收：

- 各兵种武器在 Idle / Move / Jump / Attack / Hit 中稳定跟随。
- 武器朝向和手部位置可信，不出现明显悬空或穿模。
- 骑兵 Rider 挂载武器时仍能跟随马背骨骼。
- 主将不显示武器。
...
- 弓兵远程吃子时能看到箭矢从攻击者飞向被吃棋子。
- 主将远程吃子时能看到法术表现从主将指向被吃棋子。
