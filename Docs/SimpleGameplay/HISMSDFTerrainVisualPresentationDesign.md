...
它不试图焊接不同 Static Mesh 的顶点，也不要求运行时 Dynamic Mesh 或连续地表碰撞代理。连续性由“同一球面方向得到同一材质、同一水文、同一 Cell 归属”保证；网格之间仍可因轮廓、阴影、AO、法线和遮挡关系存在差异，这些是资产和摆放算法需要控制的对象。
...
资产生产继续演进 [TerraTerrainDecorGeneratorDesign.md](TerraTerrainDecorGeneratorDesign.md)，但运行时摆放与资产生成必须分离：插件离线生成 Nanite Static Mesh；游戏运行时只选择、变换和实例化资产，不生成 Static Mesh。
...
| Ridge | 短/中/长条脊顶、破碎裸岩、侧向不对称 | MountainRidgeSegment 的球面弧、链路位置、稳定 seed。 |
| Cliff/Fault | 层状崖壁、断层、侵蚀台阶 | 山脊侧向、局部坡向、链端或断裂节点。 |
| Peak | 单峰、双峰、断峰、尖塔 | 山脊链汇合、转折和高值节点。 |
...
验收：多资产山脊从远景读为连片山系；近景有可辨识尖峰、岩层和断层；跨 Cell 山体保持点击/hover 正确；不加载独立 Decor 时，地形外壳仍完整覆盖球面。
...
- 通过 `AssetBlendMask` 让 Ridge/Cliff/Peak 的底部逐渐收敛到基础地表的颜色、粗糙度和法线，避免“独立资产插进地表”的材质接缝。
