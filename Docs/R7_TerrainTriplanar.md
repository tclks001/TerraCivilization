...
| 症状 | 根因 | 修复 |
| --- | --- | --- |
| **整球 Lit 模式漆黑（Unlit 模式纹理正常显示）⭐**（R7 经典踩坑） | **手填顶点法线方向错误**：UE5 是左手系 + CCW frontface（[`D3D12State.cpp:356`](../../Program%20Files/Epic%20Games/UE_5.8/Engine/Source/Runtime/D3D12RHI/Private/D3D12State.cpp) `FrontCounterClockwise = true`），CCW 三角形对球外渲染时 face_normal_LH **指向球心**；漫反射用 `saturate(dot(N, L))`（[`ForwardLightingCommon.ush:387-392`](../../Program%20Files/Epic%20Games/UE_5.8/Engine/Shaders/Private/ForwardLightingCommon.ush)）要求 N 与 L 同侧。直觉手填 `Normal = UnitCenter`（朝外）与 face_normal_LH 反向，`dot(N, L) ≤ 0` 大多数像素被 clamp 到 0 → 整球漆黑。R1~R6 用 Unlit 不消费法线所以未暴露 | 把法线交给 `KismetProceduralMeshLibrary::CalculateTangentsForMesh` 自动生成（推荐）；或手填 `-UnitCenter`（朝内、加注释）。**禁止**直接写 `Normal = UnitCenter`。详见 [SphereTopologyReference.md §11](SphereTopologyReference.md) / [SphericalSDFTerrainDesign.md §11.2](SphericalSDFTerrainDesign.md#112-pmcptg-渲染契约顶点法线与-ue5-光照约定) |
